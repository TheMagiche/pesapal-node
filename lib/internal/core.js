...
const myrequest = require('require');

let prepXML = function(xml){
    let php  = require('locutus/php/strings/htmlentities');
    return php.htmlentities(xml,"ENT_QUOTES");
}
...
class PostPesaPalDirectOrderV4{
    ...
    */
    ...
    setupPayment(reference,customerDetails,description, orders){
        ...
        let pesapalRequestData = model.CustomerOrder({
            reference: reference,
        ...
        let params = {
            ...
            'pesapal_request_data': prepXML(pesapalRequestData)
        }       
...
class QueryPaymentStatus{
    ...
    */
    constructor(consumer_key, consumer_secret, debug){
        this.consumer_key = consumer_key;
        this.consumer_secret = consumer_secret;
        this.debug =  debug;
    }
    ...
    responseData(merchant_ref, merchant_tracking_id){
        ...
        let req = request.signed_url;
        return new Promise(function(resolve, reject){
            var data = null;
            myrequest.get({url: req}, function(error, response, body){
            if (!error) {
                if (body.indexOf("Problem:") == 0) {
                    error = new Error(body);
                } 
                else if (response.statusCode != 200){
                    error = new Error(`Something went wrong; Could not fetch payment. (Error ${respoonse.statusCode})`);
                }
                else {
                    data = body
                }
        ...
        });  // return Promise with expeced values
    }
}
...
class QueryPaymentDetails{
    ...
    */
    ...
    responseData(merchant_ref, merchant_tracking_id){
        ...
        });  // return Promise with expeced values  // return Promise with expeced values
    }
}
